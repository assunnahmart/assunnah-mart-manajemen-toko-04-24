...
const Konsinyasi = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createBarang.mutateAsync({
        ...formData,
        harga_beli: formData.harga_beli.toString(),
  ...
  const getStokStatus = (stokSaatIni: number, stokMinimal: number) => {
    if (stokSaatIni <= stokMinimal) {
      return { variant: "destructive", text: "Stok Habis" };
    } else if (stokSaatIni <= stokMinimal * 1.5) {
      return { variant: "outline", text: "Stok Rendah" };
    }
    return { variant: "default", text: "Stok Aman" };
  };
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            ...
            <div>
              ...
              <p className="font-medium">{formatRupiah(barang.harga_beli)}</p>
            </div>
            <div>
              <span className="text-gray-500">Jual:</span>
              <p className="font-medium">{formatRupiah(barang.harga_jual)}</p>
            </div>
          </div>
