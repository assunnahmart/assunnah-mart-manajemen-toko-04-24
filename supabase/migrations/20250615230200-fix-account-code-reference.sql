...
-- Fix the account code reference in payment function
...
BEGIN
  ...
  -- Debit: Cash (Asset account) - Fixed column reference
  PERFORM create_general_ledger_entry(
    p_payment_date,
    (SELECT id FROM chart_of_accounts WHERE kode_akun = '1110' LIMIT 1), -- Cash account - use kode_akun not account_code
    p_amount, 0,
  ...
  -- Credit: Accounts Receivable (Asset account) - Fixed column reference
  PERFORM create_general_ledger_entry(
    p_payment_date,
    (SELECT id FROM chart_of_accounts WHERE kode_akun = '1120' LIMIT 1), -- Accounts Receivable - use kode_akun not account_code
    0, p_amount,
    'Pembayaran piutang dari ' || p_pelanggan_name,
    'customer_payment', kas_transaction_id,
    NULL, p_pelanggan_name, 'cash', p_kasir_name, 'pembayaran', p_reference_number
