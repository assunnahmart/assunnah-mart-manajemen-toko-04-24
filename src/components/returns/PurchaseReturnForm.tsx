...
import { useCreatePurchaseReturn } from '@/hooks/useReturTransactions';
...
const PurchaseReturnForm = () => {
  ...
  const handleSubmit = async () => {
    if (!selectedTransactionId || !userKasir || items.length === 0) {
      toast({
        title: "Error",
    ...
    try {
      await createReturn.mutateAsync({
        retur: {
          transaksi_pembelian_id: selectedTransactionId,
          supplier_id: selectedTransaction?.supplier_id,
      ...
      toast({
        title: "Retur pembelian berhasil",
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Retur Pembelian</CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        {/* Transaction Selection */}
        <div className="grid grid-cols-2 gap-4">
          <div>
            <Label htmlFor="transaction">Transaksi Pembelian</Label>
              ...
              <SelectContent>
                {purchaseTransactions?.map((transaction) => (
                  <SelectItem key={transaction.id} value={transaction.id}>
                    {transaction.nomor_transaksi} - {transaction.supplier?.nama}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
